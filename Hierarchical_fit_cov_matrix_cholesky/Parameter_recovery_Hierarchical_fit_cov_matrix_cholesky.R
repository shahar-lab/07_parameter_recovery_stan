...
beta_aux_var  = 0.05
corr_alpha_beta = 0.2
cov_alpha_beta  =sqrt(alpha_aux_var)*sqrt(beta_aux_var)*corr_alpha_beta
...
print(paste('true alpha population parm is', alpha_mu,'true sample mean is',mean(true.parms[,1])))
print(paste('true beta population parm is', beta_mu,'true sample mean is',mean(true.parms[,2])))
print(paste('true alpha population parm is', alpha_aux_mu,'true sample mean is',mean(auxiliary_parameters[,1])))
print(paste('true beta population parm is', beta_aux_mu,'true sample mean is',mean(auxiliary_parameters[,2])))
print(paste('true alpha aux var parm is', alpha_aux_var,'true sample mean is',var(auxiliary_parameters[,1])))
print(paste('true beta aux var parm is', beta_aux_var,'true sample mean is',var(auxiliary_parameters[,2])))
print(paste('true corr between alpha and beta aux parms is', corr_alpha_beta,'true sample mean is',cor(auxiliary_parameters[,1],auxiliary_parameters[,2])))
...
#fit stan model   
rl_fit<- stan(file = "Hierarchical_cov_matrix_cholesky.stan", data=model_data, iter=2000,chains=4,cores =2) #iter - number of MCMC samples 
...
alpha_aux_sigma_recovered = summary(rl_fit , pars=c("tau[1]"))$summary[,1]
beta_aux_sigma_recovered = summary(rl_fit , pars=c("tau[2]"))$summary[,1]
cov_alpha_beta_recovered = summary(rl_fit , pars=c("sigma_matrix[2,1]"))$summary[,1]


#compare recovered to true population parameters
print(paste('true alpha population parm is', alpha_mu,'true sample mean is',mean(true.parms[,1]),'and recovered is',inv.logit(alpha_aux_mu_recovered)))
print(paste('true beta population parm is', beta_mu,'true sample mean is',mean(true.parms[,2]),'and recovered is',exp(beta_aux_mu_recovered)))
print(paste('true alpha aux population parm is', alpha_aux_mu,'true sample mean is',mean(auxiliary_parameters[,1]),'and recovered is',alpha_aux_mu_recovered))
print(paste('true beta aux population parm is', beta_aux_mu,'true sample mean is',mean(auxiliary_parameters[,2]),'and recovered is',beta_aux_mu_recovered))
print(paste('true alpha aux var parm is', alpha_aux_var,'true sample mean is',var(auxiliary_parameters[,1]),'and recovered is',alpha_aux_sigma_recovered))
print(paste('true beta aux var parm is', beta_aux_var,'true sample mean is',var(auxiliary_parameters[,2]),'and recovered is',beta_aux_sigma_recovered))
print(paste('true corr between alpha and beta aux parms is', corr_alpha_beta,'true sample mean is',cor(auxiliary_parameters[,1],auxiliary_parameters[,2]),
