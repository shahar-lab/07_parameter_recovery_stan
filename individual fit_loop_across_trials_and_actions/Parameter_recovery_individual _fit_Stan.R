...
Nalt<-c(2)               #number of alternatives
Ntrl<-c(20)          #number of trials
...
df<-  lapply(1:length(Nalt), function(alt) {
    ...
    mclapply(1:N,function(s)               {
                   sim.block(Ntrl[trl],Nalt[alt],true.parms$alpha[s],true.parms$beta[s],rndwlk)},
                   mc.cores=2)})})
...
mclapply(1:N,function(subject)         {
         ...
         rl_fit<- stan(file = "rl_basic.stan", data=model_data, iter=1000,chains=1,cores =1) #iter - number of MCMC samples 
